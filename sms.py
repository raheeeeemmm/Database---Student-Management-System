...
def add_data():
    if idEntry.get() == '' or nameEntry.get() == '' or phoneEntry.get() == '' or emailEntry.get() == '' or addressEntry.get() == '' or genderEntry.get() == '' or dobEntry.get() == '':
        ...
        try:
            ...
            messagebox.showerror('Error', 'ID must be an integer', parent=screen)
            return
        
        try:
            # Validate phone number as integer and length not exceeding 11 digits
            phone_value = int(phoneEntry.get())
            if len(phoneEntry.get()) > 11:
                messagebox.showerror('Error', 'Phone number must not exceed 11 digits', parent=screen)
                return
        except ValueError:
            messagebox.showerror('Error', 'Phone number must be an integer', parent=screen)
            return
        
        # Validate name as string
        name_value = nameEntry.get()
        if not name_value.isalpha():
            messagebox.showerror('Error', 'Name must contain only alphabetic characters', parent=screen)
            return
        
        # Validate email format
        email_value = emailEntry.get()
        if not email_value.endswith('@gmail.com'):
            messagebox.showerror('Error', 'Enter a valid email address (e.g., example@gmail.com)', parent=screen)
            return
        
        # Validate D.O.B format as integers (day, month, year)
        dob_value = dobEntry.get()
        try:
            day, month, year = map(int, dob_value.split('/'))
        except ValueError:
            messagebox.showerror('Error', 'Enter a valid D.O.B format (e.g., DD/MM/YYYY)', parent=screen)
            return
        
        # Validate gender as either "Male" or "Female"
        gender_value = genderEntry.get().capitalize()
        ...
        try:
            ...
            mycursor.execute(query, (id_value, name_value, phone_value, email_value, addressEntry.get(),
                                    gender_value, dob_value, date, currenttime))
            con.commit()
